                    ...
                    {loading ? (
                            ...
                            <div className="w-full max-w-4xl">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {((title?.toLowerCase().includes('integrativa') || title?.toLowerCase().includes('homeopática')) ? [
                                        { name: 'Dr. Samuel Hahnemann', specialty: 'Médico Homeopata Clássico', icon: '🌿', delay: '0s', color: 'teal' },
                                        { name: 'Dr. Li Wei', specialty: 'Especialista em MTC', icon: '☯️', delay: '0.2s', color: 'green' },
                                        { name: 'Dra. Elena Petrov', specialty: 'Medicina Holística', icon: '🍃', delay: '0.4s', color: 'emerald' },
                                        { name: 'Dr. Hiroshi Tanaka', specialty: 'Acupunturista Tradicional', icon: '📍', delay: '0.6s', color: 'lime' },
                                        { name: 'Dr. Fernando Alves', specialty: 'Farmacêutico PhD', icon: '💊', delay: '0.8s', color: 'blue' },
                                        { name: 'Dr. Ricardo Souza', specialty: 'Fisioterapeuta Integrativo', icon: '🧘', delay: '1s', color: 'orange' }
                                    ] : [
                                        ...
                                        { name: 'Dr. Paulo Mendes', specialty: 'Clínico Geral Sênior', icon: '🩺', delay: '1s', color: 'slate' }
                                    ]
                                    ).map((doctor, idx) => (
                                        <div
                                            key={idx}
                        ...
                        <div className="space-y-6 max-w-4xl mx-auto">
                            ...
                            {data.alteredExamsAnalysis && (
                                ...
                                >
                                    <div className="prose prose-slate max-w-none text-slate-600 leading-loose text-left text-sm bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                        <div dangerouslySetInnerHTML={{ __html: (data?.alteredExamsAnalysis || '').replace(/\. /g, '.<br/><br/>').replace(/\n/g, '<br/>').replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900 font-bold">$1</strong>') }} />
                                    </div>
                                </AccordionSection>
                            )}
                            ...
                            >
                                <div className="space-y-6">
                                    ...
                                    <div className="prose prose-slate max-w-none text-slate-600 leading-loose text-left text-sm bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-6">
                                        <div dangerouslySetInnerHTML={{ __html: (data?.discussionSummary || '').replace(/\. /g, '.<br/><br/>').replace(/\n/g, '<br/>').replace(/"(.*?)"/g, '<span class=\'italic text-slate-800 font-medium\'>"$1"</span>').replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900">$1</strong>') }} />
                                    </div>
                                ...
                                <div className="space-y-8 text-left">
                                    ...
                                    <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100/50">
                                        ...
                                        <div className="prose prose-slate max-w-none text-slate-700 text-sm leading-relaxed">
                                            <div dangerouslySetInnerHTML={{ __html: (data?.recommendations || '').replace(/\. /g, '.<br/><br/>').replace(/\n/g, '<br/>').replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900 font-bold">$1</strong>') }} />
                                        </div>
                                    </div>
                                </div>
                            </AccordionSection>
